...
import json
from CDZF_Data.items import Revoke_Item
import re

class RevokeSpider(Spider):
    name = 'revoke'
    allowed_domains = ['http://www.cddata.gov.cn/odweb/catalog/catalogDetail.htm?cata_id=Y6UWkS44EeioZYUL_fhovg']
    start_urls = ['http://http://www.cddata.gov.cn/odweb/catalog/catalogDetail.htm?cata_id=Y6UWkS44EeioZYUL_fhovg/']
    custom_settings = {  # 修改settings文件中Pipeline的值
        'ITEM_PIPELINES': {'CDZF_Data.pipelines.Revoke_MysqlTwistPipeline': 300}
    }

    def __init__(self):
        self.start_url = 'http://www.cddata.gov.cn/odweb/catalog/catalogDetail.htm?cata_id=Y6UWkS44EeioZYUL_fhovg'
        self.headers = {  # 请求头
    ...
    def start_parse(self, response):  # 获取数据总量
        ...
        if type(data_num) == float:
            ...
            for i in range(1, data_num + 1):
                data = {  # 请求头
                    'cata_id': "Y6UWkS44EeioZYUL_fhovg",
                    'conf_type': "2",
                    'where': "",
                    '_search': "false",
                    'nd': "1542337300712",
    ...
    def parse_info(self, response):  # 解析服务器返回的数据，使用正则表达式处理特殊符号！
        ...
        if data:
            ...
            for iter in info:
                ...
                revoke_date = iter.get('revoke_date', '')
                if revoke_date is None:
                    items['revoke_date'] = revoke_date
                else:
                    items['revoke_date'] = re.sub(regex, ' ', revoke_date)
